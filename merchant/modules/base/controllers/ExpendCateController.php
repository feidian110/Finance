...
class ExpendCateController extends BaseController
{
    use MerchantCurd;

    public $modelClass = Category::class;

    public $type = FinanceCateEnum::EXPEND;
    ...
    public function actionCreate()
    {
        ...
        if( Yii::$app->request->isPost ){
            ...
            if( $model->load($data) && $model->save() ){
                return $this->message('支出类别添加成功！', $this->redirect(['index']), 'success');
            }
    ...
    public function actionEdit()
    {
        ...
        $model = Category::findOne(['id' => $id,'merchant_id' =>Yii::$app->services->merchant->getId(),'type'=>$this->type]);
        if( $model == null ){
        ...
        if( Yii::$app->request->isPost ){
            $data = Yii::$app->request->post();
            if( $model->load($data) && $model->save() ){
                return $this->message('支出类别更新成功！', $this->redirect(['index']), 'success');
            }
            return $this->message($this->getError($model), $this->redirect(['index']), 'error');
        }
        return $this->renderAjax( $this->action->id,[
